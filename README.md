...
- 帰還側：R11 (10k) ＋ Pot (100k)
  ...
  - Potの2番（ワイパー） から IC1 Pin 9 (−) へ戻る。
  - ※これで「出力→入力」のループ（帰還）が完成し、ゲイン制御が機能します。
...
###GR Meter 入力回路（反転・スケーリング）
LM3914の前段に、ゲイン調整機能付きの反転アンプを追加します。
```text
[CV Out (負電圧)]
      │
      R_in (100k)
      │
      ├─────── [OP_M (Meter Op-amp) -入力]
      │           │
      │      ┌────┴──── [GR_Trim Pot (100k B)] ──┐
      │      │           (ゲイン調整用)           │
      │      │                                   │
      │      └───────── [OP_M 出力] ─────────────┴─── [LM3914 Sig In (Pin 5)]
      │                   │
    [GND] ────────── [OP_M +入力]
```
- CVの負電圧（例：-1.5V）を正電圧（例：+1.5V）に変換します。
- スケーリング: GR_Trim（半固定抵抗）を設けることで、VCAの圧縮量とLEDの点灯具合を微調整（キャリブレーション）できます。例えば、「最大圧縮時にちょうど10番目のLEDが光る」ように設定可能です。
### LM3914 基本配線
```text
[TO_METER_IN] (From SC Buffer / CV GRメーター入力：0V〜正電圧（専用反転バッファ経由）
      │
      │ ※CVが負電圧の場合は反転アンプを通すか、
